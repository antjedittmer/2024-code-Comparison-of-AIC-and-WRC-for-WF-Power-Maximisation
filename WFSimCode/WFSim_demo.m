function [sol_array,JR,JQ,fileName,mpc] = WFSim_demo(R,refstairs,measured,KoopmanStates,PolyLiftingFunction,...
...
if nargin < 7
    %ControlSetStr = 'sowfa_9turb_apc_alm_turbl';
...
if nargin < 9 || isempty(vinfStr)
    vinfStr = ''; %'_Vinf'; %'';%'_Vinf'; %_Vinf'; %alternativ '';
end
...
%eval(['Wp = layoutSet_sowfa_9turb_apc_alm_turbl',vinfStr,';']); %alternativ; %; % Choose which scenario to simulate. See 'layoutDefinitions' folder for the full list.
...
elseif sol.update
    ...
    time1 = time(time >= 120 & time <= 1200);
    %[JR,JQ] = plotWFSimTimeseriesLatex3(Wp,sol_array,controller,Power,CT_prime,Phi,V,JR,time1,measured,figName,dirFig,mpc);
    [JR,JQ] = plotWFSimTimeseriesLatex(Wp,sol_array,controller,Power,CT_prime,Phi,V,JR,time1,measured,figName,dirFig,mpc);
end