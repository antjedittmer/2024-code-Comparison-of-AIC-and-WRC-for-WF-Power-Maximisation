...
% legend('$P_{1,WFSim}$','$P_{2,WFSim}$','$P_{T,WFSim}$','$P_{T,WFSim,max}$)',...
...
legend({'$P1_{WFSim}$','$P2_{WFSim}$','$PT_{WFSim}$','$PT_{max,WFSim}$',...
    '$P1_{Gauss}$','$P2_{Gauss}$','$PT_{Gauss}$','$PT_{max,Gauss}$'}',...
    'Interpreter','Latex','Location','EastOutside');

%sprintf('max P: $\\gamma$ = %d [deg]', phi1K(vecIdx)),...
strName = sprintf('PowerVsYaw_1');
...
%% Plot one image for two yaw angles horizonally 
nF = figure(nF.Number +1); %#ok<SAGROW>
WFSim_animation_compare_h( Wp,solCell,nF,deltaCell,sigmaYCell,xDistVecCell);
filenamepng1 = 'gamma0_20_AngleCenterH';
set(gcf,'Name', filenamepng1);
print(gcf,fullfile(dirFig,filenamepng1), '-dpng');
print(gcf,fullfile(dirFig,filenamepng1), '-depsc');
