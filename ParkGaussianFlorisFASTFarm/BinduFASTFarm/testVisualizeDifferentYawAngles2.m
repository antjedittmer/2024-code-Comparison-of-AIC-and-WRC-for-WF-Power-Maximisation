...
%% Set path for mat file genearted outputs
plotDir = fileparts(mfilename('fullpath'));
matlabDir = fileparts(plotDir);
parentDir = fileparts(matlabDir);
matdataDir = fullfile(parentDir,'DataInFast');
matfileName = fullfile(matdataDir, 'genPwrMat.mat');
...
if exist(matfileName,'file') == 2
    ...
    %% Set path for OpenFAST Matlab plot files and outputs
...
noF = plotFigurePwrMntsBT2(xlabelStrTex,ylabelStr, titleStr, noYaw, timeVec, ...
    genPwrFarm,genPwrMatrix, towerBMxMatrix,towerBMyMatrix, towerBMzMatrix,...
    towerMxMatrix,towerMyMatrix, towerMzMatrix,outmidName,dirFig,noF);
