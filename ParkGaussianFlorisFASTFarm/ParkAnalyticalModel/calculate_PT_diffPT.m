function [PT, dP] = calculate_PT_diffPT(a_vec, c, rho, A, Vinf)
...
for idx = 1:m % loop over turbines
    ...
    V(idx) =  (1 - 2 * sqrt( sum( (c(idx,:).*a_vec).^2 ))); %disp(V)
    Cp     = 4*a_vec(idx)*(1-a_vec(idx)).^2; % power coefficient
    
    P(idx) = K_v * Cp * V(idx)^3;
end
